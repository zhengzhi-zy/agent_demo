...
from utils.config_handler import prompts_conf as pc, prompts_conf
...
def load_system_prompts():
    ...
    try:
        ...
        logger.error(logger.error(f"[load_system_prompts])解析系统提示词出错，{str(e)}"))
        raise e
...
def load_rag_prompts():
    ...
    try:
        ...
        logger.error(logger.error(f"[load_rag_prompts])解析RAG提示词出错，{str(e)}"))
        raise e
...
def load_report_prompts():
    ...
    try:
        ...
        logger.error(logger.error(f"[load_report_prompts])解析报告提示词出错，{str(e)}"))
        raise e
